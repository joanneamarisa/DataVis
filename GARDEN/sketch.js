...
function draw() {
  ...
  let subhead =
    "is a visual representation of a mother-daughter conversation. Welcome to this space. This is a data visualisation of WhatsApp text messages shared between my mother and I, spanned across thirty days (March-April 2020). Hover anywhere over the garden to see what each element represents!";
  fill(0);
  textFont("garamond", 15);
  text(subhead, 60, 80, 375, 150);
  text(
    "Song: Edelweiss by Irwin Kostal Press to play!",
    windowWidth - 300,
    60,
    225,
    200
  ...
  let name =
    "Joanne Amarisa (S3652802) Visualising Data Through Art RMIT University 2020";
  ...
  for (let i = 1; i < listLength; i++) {
    ...
    //minMaxCheck shows --> min:-414.5max:575.5  

    let lineYMap = map(lineY, minHeight, maxHeight, 50, Ground);
    
    flowerSpacing = width / 32;
    stroke(94, 119, 3);
    strokeWeight(1.5);
...
function minMaxCheck() {
  ...
  for (let i = 1; i < listLength; i++) {
    ...
    y += 7.5;
    lineY = height - 70 - y;
    // console.log("minMax: Day " + day + ":" + lineY);
...
function HoverA(mx, my) {
  ...
  for (let i = 0; i < leafCoordinates.length; i++) {
    let leaf = leafCoordinates[i];
    if (
      mx > leaf.x - extraRadius &&
      mx < leaf.x + extraRadius &&
      my > leaf.y - extraRadius &&
      my < leaf.y + extraRadius
    ) {
      withinRadius = true;
      break;
  ...
  if (withinRadius) {
    ...
    text(
      "*Leaves growing to the left represent my text message, and the right represent my mothers.",
      60,
      200
    );
  } else {
...
function HoverB(mx, my) {
  ...
  for (let i = 0; i < careCoordinates.length; i++) {
    let care = careCoordinates[i];
    if (
      mx > care.x - extraRadius &&
      mx < care.x + extraRadius &&
      my > care.y - extraRadius &&
      my < care.y + extraRadius
    ) {
      withinRadius = true;
      break;
...
function HoverD(mx, my) {
  ...
  for (let i = 0; i < loveCoordinates.length; i++) {
    let love = loveCoordinates[i];
    if (
      mx > love.x - extraRadius &&
      mx < love.x + extraRadius &&
      my > love.y - extraRadius &&
      my < love.y + extraRadius
    ) {
      withinRadius = true;
      break;
...
function HoverE(mx, my) {
  ...
  for (let i = 0; i < prayCoordinates.length; i++) {
    let pray = prayCoordinates[i];
    if (
      mx > pray.x - extraRadius &&
      mx < pray.x + extraRadius &&
      my > pray.y - extraRadius &&
      my < pray.y + extraRadius
    ) {
      withinRadius = true;
      break;
...
function HoverF(mx, my) {
  ...
  for (let i = 0; i < flowerJoCoordinates.length; i++) {
    let flowerJo = flowerJoCoordinates[i];
    if (
      mx > flowerJo.x - extraRadius &&
      mx < flowerJo.x + extraRadius &&
      my > flowerJo.y - extraRadius &&
      my < flowerJo.y + extraRadius
    ) {
      withinRadius = true;
      break;
  ...
  if (withinRadius) {
    ...
    fill(0, textFill);
    text(
      "- RED FLOWERS symbolise days where I spoke the most",
      mouseX,
      mouseY + 10
    );
  } else {
...
function HoverG(mx, my) {
  ...
  for (let i = 0; i < flowerMomCoordinates.length; i++) {
    let flowerMom = flowerMomCoordinates[i];
    if (
      mx > flowerMom.x - extraRadius &&
      mx < flowerMom.x + extraRadius &&
      my > flowerMom.y - extraRadius &&
      my < flowerMom.y + extraRadius
    ) {
      withinRadius = true;
      break;
  ...
  if (withinRadius) {
    ...
    fill(0, textFill);
    text(
      "- YELLOW FLOWERS symbolise days where my mother spoke the most",
      mouseX,
      mouseY + 10
    );
  } else {
